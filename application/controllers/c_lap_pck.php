...
class c_lap_pck extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			$d['judul']="Laporan AR Packing";
			$d['class'] = "laporan";
			
			$d['content']= 'laporan/v_lap_pck';
	...
	public function cari_data()
	{
		...
		if(!empty($cek) && $level=='admin'){
			...
			echo $this->load->view('laporan/v_lap_pck_view',$dt);
	...
	public function print_excel()
	{
		...
		if(!empty($cek) && $level=='admin'){
			...
			if($r>0){
				...
				header("Content-Disposition: attachment; filename=pck_transaksi_".$tgl_awal.'_'.$tgl_akhir.".xls");
				header("Pragma: no-cache");
				header("Expires: 0");
			?>
            <p>LAPORAN AR PACKING</p>
