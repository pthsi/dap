...
class c_glz extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek) && $level=='admin'){
			...
			$d['data'] = $this->db->query("select a.id_fg, a.tgl_input, b.nama_lengkap as pic, a.jenis, a.plan, a.aktual, 		
							a.inputer from gl_trans a join fg_users b on a.pic = b.username order by a.tgl_input desc");
	...
	public function tambah()
	{
		...
		$level = $this->session->userdata('level');
		if(!empty($cek) && $level=='admin'){
			$d['judul']="Input Glasir";
			$d['class'] = "transaksi";
			
			$id_fg = $this->model_global->maxGlz();
			$d['id_fg'] = $id_fg;
			
			$d['content'] = 'glz/v_gl_form';
			$this->load->view('home',$d);
		}else{
			redirect('login','refresh');
		}
	...
	public function cari()
	{
		...
		if(!empty($cek) && $level=='admin'){
			...
			$q = $this->db->get_where("gl_trans",$id_fg);
			$row = $q->num_rows();
			if($row>0){
	...
	public function simpan()
	{
		...
		if(!empty($cek) && $level=='admin'){
			...
			$q = $this->db->get_where("gl_trans",$id);
			$row = $q->num_rows();
			if($row>0){
				$dt['tgl_update'] = date('Y-m-d h:i:s');
				$this->db->update("gl_trans",$dt,$id);
				echo "Data Sukses diUpdate";
			}else{
				$dt['tgl_insert'] = date('Y-m-d h:i:s');
				$this->db->insert("gl_trans",$dt);
