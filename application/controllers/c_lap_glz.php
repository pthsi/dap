...
class c_lap_glz extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			$d['judul']="Laporan Rekap Glasir";
			$d['class'] = "laporan";
                        
			$d['content']= 'laporan/v_lap_glz';
	...
	public function cari_data()
	{
		...
		if(!empty($cek) && $level=='admin'){
			...
			$query = "SELECT a.tgl_input,b.nama_lengkap,a.jenis,a.plan,a.aktual,a.inputer, truncate((a.aktual/a.plan),2)*100 as ar
						FROM gl_trans as a JOIN fg_users as b
			...
			echo $this->load->view('laporan/v_lap_glz_view',$dt);
			...
			if($r>0){
			  ...
			  $pdf->AddPage("P","A4");
				//foreach($data->result() as $t){
					...
					$pdf->Cell($w,$h,'Laporan Rekap Glasir',0,1,'C');
					$pdf->Cell($w,$h,$tgl_awal.' - '.$tgl_akhir,0,1,'C');
					$pdf->Ln(5);
										
					
					$l=10;
					$w = array(10,30,30,30,25,25,10,30);
					...
					$pdf->Cell($w[3],$h,'Jenis Glasir',1,0,'C',$fill);
				...
				//}
				$pdf->Output('glz_transaksi_'.$tgl_awal.'-'.$tgl_akhir.'.pdf','D');		
			}else{
	...
	public function print_excel()
	{
		...
		if(!empty($cek) && $level=='admin'){
			...
			if($r>0){
				...
				header("Content-Disposition: attachment; filename=glz_transaksi_".$tgl_awal.'_'.$tgl_akhir.".xls");
				header("Pragma: no-cache");
				header("Expires: 0");
			?>
            <p>LAPORAN REKAP GLASIR</p>
