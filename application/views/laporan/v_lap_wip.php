...
$(document).ready(function(){
	...
	function cari_data(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/c_lap_wip/cari_data",
			data	: string,
			cache	: false,
	...
	$("#cetak_pdf").click(function(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/c_lap_wip/cetak_pdf",
			data	: string,
			cache	: false,
			success	: function(data){
				if(data=='Sukses'){
					window.location.assign("<?php echo site_url();?>/c_lap_wip/print_pdf");
				}else{
					$.gritter.add({
						title: 'Peringatan..!!',
						text: data,
	...
	$("#cetak_excel").click(function(){
		...
		$.ajax({
			...
			success	: function(data){
				if(data=='Sukses'){
					window.location.assign("<?php echo site_url();?>/c_lap_wip/print_excel");
				}else{
					$.gritter.add({
						title: 'Peringatan..!!',
						text: data,
                    ...
                    <div class="control-group">
                          ...
                          <div class="input-append">
                               ...
                               <span class="add-on">
                                ...
                                <select name="jns_wip" id="jns_wip" class="span2">
                                    <option value="">All</option>
                                    <?php
                                    $data = $this->model_data->ambil_jns_wip();
                                    foreach($data->result() as $dt){
                                    ?>
                                    <option value="<?php echo $dt->jns_wip;?>"><?php echo $dt->jns_wip;?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
