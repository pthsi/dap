...
$(document).ready(function(){
	...
	function cari_data(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/c_lap_fgar/cari_data",
			data	: string,
			cache	: false,
	...
	$("#cetak_pdf").click(function(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/c_lap_fgar/cetak_pdf",
			data	: string,
			cache	: false,
			success	: function(data){
				if(data=='Sukses'){
					window.location.assign("<?php echo site_url();?>/c_lap_fgar/print_pdf");
				}else{
					$.gritter.add({
						title: 'Peringatan..!!',
						text: data,
	...
	$("#cetak_excel").click(function(){
		...
		$.ajax({
			...
			success	: function(data){
				if(data=='Sukses'){
					window.location.assign("<?php echo site_url();?>/c_lap_fgar/print_excel");
				}else{
					$.gritter.add({
						title: 'Peringatan..!!',
						text: data,
                               ...
                               <span class="add-on">
                                ...
                                <select name="pengiriman" id="pengiriman" class="span4">
                                    <option value="">All</option>
                                     <?php
                                            foreach($kota as $k){
                                                echo "<option value='".$k->id_wip."'>".$k->ket." - ".$this->model_global->tgl_ymo($k->time_release)."</option>";
