...
$(document).ready(function(){
	...
	function cari_data(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/c_lap_fg/cari_data",
			data	: string,
			cache	: false,
	...
	$("#cetak_pdf").click(function(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/c_lap_fg/cetak_pdf",
			data	: string,
			cache	: false,
			success	: function(data){
				if(data=='Sukses'){
					window.location.assign("<?php echo site_url();?>/c_lap_fg/print_pdf");
				}else{
					$.gritter.add({
						title: 'Peringatan..!!',
						text: data,
	...
	$("#cetak_excel").click(function(){
		...
		$.ajax({
			...
			success	: function(data){
				if(data=='Sukses'){
					window.location.assign("<?php echo site_url();?>/c_lap_fg/print_excel");
				}else{
					$.gritter.add({
						title: 'Peringatan..!!',
						text: data,
                    ...
                    <div class="control-group">
                          ...
                          <div class="input-append">
                               ...
                               <span class="add-on">
                                ...
                                <select name="pengiriman" id="pengiriman" class="span2">
                                    <option value="">All</option>
                                     <?php
                                            foreach($kota as $k){
