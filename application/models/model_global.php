...
class Model_global extends CI_Model {
	...
	function get_chart(){
        ...
        function get_chart_ar(){
		$query = "SELECT t.tgl_input as kategori,
					 (select x.total from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where x.tgl_input = t.tgl_input
					   AND y.ket = 'lokal kw1' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS tot_lok1,
					 (select x.total from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where x.tgl_input = t.tgl_input
					   AND y.ket = 'lokal kw2' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS tot_lok2,
					 (select x.total from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where x.tgl_input = t.tgl_input
					   AND y.ket = 'ekspor' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS tot_eks,
					 (SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'lokal kw1' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS kum_lok1,
					 (SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'lokal kw2' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS kum_lok2,
					 (SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'ekspor' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS kum_eks,
					 (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw1' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS tar_lok1,
					 (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw2' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS tar_lok2,
					 (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'ekspor' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS tar_eks,
					   
					   truncate((SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'ekspor' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())
					   /
					   (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'ekspor' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_eks,
					   
					   truncate((SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'lokal kw1' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())
					   /
					   (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw1' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_kw1,
					   
					   truncate((SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'lokal kw2' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())
					   /
					   (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw2' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_kw2,
						 
						
						truncate((select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'ekspor' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())
						 /
						 (select count(DISTINCT(day(x.tgl_input))) from fg_trans_ar x
						 where x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()),2)AS avg_tot_eks,
						 
						 truncate((select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw1' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())
						 /
						 (select count(DISTINCT(day(x.tgl_input))) from fg_trans_ar x
						 where x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()),2)AS avg_tot_lok1,
						 
						 truncate((select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw2' 
						 and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())
						 /
						 (select count(DISTINCT(day(x.tgl_input))) from fg_trans_ar x
						 where x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()),2)AS avg_tot_lok2
					   
					FROM fg_trans_ar t where t.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()
					group BY day(t.tgl_input)";
        ...
        function get_chart_jc(){
		$query = "SELECT t.tgl_input as kategori, pic, 
					 (select x.plan from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'casting' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS pln_cas,
					 (select x.aktual from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'casting' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS akt_cas,
					 truncate((select x.aktual from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'casting' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())/
					 (select x.plan from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'casting' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_cas,
					   
					 (select x.plan from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'jiggering' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS pln_jig,
					 (select x.aktual from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'jiggering' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS akt_jig,
					 truncate((select x.aktual from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'jiggering' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())/
					 (select x.plan from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'jiggering' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_jig
					FROM jc_trans t where t.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()
					group BY day(t.tgl_input)";
        ...
        function get_chart_gl(){
		$query = "SELECT t.tgl_input as kategori, pic, 
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'dipping' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS pln_dip,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'dipping' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS akt_dip,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'dipping' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'dipping' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_dip,
					   
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'konveyer' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS pln_kon,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'konveyer' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS akt_kon,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'konveyer' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'konveyer' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_kon,
					   
					   (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'mesin' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS pln_mes,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'mesin' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS akt_mes,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'mesin' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'mesin' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_mes,
					   
					   (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'spray' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS pln_spr,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'spray' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS akt_spr,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'spray' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'spray' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_spr,
					   
					    (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'waterfall' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS pln_wat,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'waterfall' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) AS akt_wat,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'waterfall' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'waterfall' and x.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW())*100,2) as ar_wat
					   
					FROM gl_trans t where t.tgl_input between DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()
					group BY day(t.tgl_input)";
...
group by month(a.periode), b.sec_des order by gt desc";
	...
	function get_chart_manual($tgl_awal,$tgl_akhir){
        ...
        function getChartManualFGR($bulan,$tahun,$grup){
            
		$query = "SELECT t.tgl_input as kategori,
					 (select x.total from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where x.tgl_input = t.tgl_input
					   AND y.ket = 'lokal kw1' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS tot_lok1,
					 (select x.total from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where x.tgl_input = t.tgl_input
					   AND y.ket = 'lokal kw2' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS tot_lok2,
					 (select x.total from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where x.tgl_input = t.tgl_input
					   AND y.ket = 'ekspor' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS tot_eks,
					 (SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'lokal kw1' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS kum_lok1,
					 (SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'lokal kw2' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS kum_lok2,
					 (SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'ekspor' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS kum_eks,
					 (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw1' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS tar_lok1,
					 (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw2' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS tar_lok2,
					 (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'ekspor' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS tar_eks,
					   
					   truncate((SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'ekspor' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)
					   /
					   (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'ekspor' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_eks,
					   
					   truncate((SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'lokal kw1' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)
					   /
					   (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw1' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_kw1,
					   
					   truncate((SELECT SUM(x.total)
						FROM fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip
					   WHERE x.tgl_input <= t.tgl_input AND y.ket = 'lokal kw2' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)
					   /
					   (select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw2' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_kw2,
						 
						
						truncate((select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'ekspor' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)
						 /
						 (select count(DISTINCT($grup(x.tgl_input))) from fg_trans_ar x
						 where YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan),2)AS avg_tot_eks,
						 
						 truncate((select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw1' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)
						 /
						 (select count(DISTINCT($grup(x.tgl_input))) from fg_trans_ar x
						 where YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan),2)AS avg_tot_lok1,
						 
						 truncate((select distinct x.target from fg_trans_ar x join wip_keramik_kat y on x.jenis = y.id_wip where y.ket = 'lokal kw2' 
						 and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)
						 /
						 (select count(DISTINCT($grup(x.tgl_input))) from fg_trans_ar x
						 where YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan),2)AS avg_tot_lok2
					   
					FROM fg_trans_ar t where YEAR(t.tgl_input) = $tahun AND MONTH(t.tgl_input) = $bulan
					group BY $grup(t.tgl_input)";
        ...
        function getChartManualJC($bulan,$tahun,$grup){
            
		$query = "SELECT t.tgl_input as kategori, pic, 
					 (select x.plan from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'casting' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS pln_cas,
					 (select x.aktual from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'casting' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS akt_cas,
					 truncate((select x.aktual from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'casting' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)/
					 (select x.plan from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'casting' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_cas,
					   
					 (select x.plan from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'jiggering' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS pln_jig,
					 (select x.aktual from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'jiggering' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS akt_jig,
					 truncate((select x.aktual from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'jiggering' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)/
					 (select x.plan from jc_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'jiggering' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_jig
					FROM jc_trans t where YEAR(t.tgl_input) = $tahun AND MONTH(t.tgl_input) = $bulan
					group BY $grup(t.tgl_input)";
        ...
        function getChartManualGL($bulan,$tahun,$grup){
            
		$query = "SELECT t.tgl_input as kategori, pic, 
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'dipping' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS pln_dip,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'dipping' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS akt_dip,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'dipping' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'dipping' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_dip,
					   
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'konveyer' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS pln_kon,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'konveyer' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS akt_kon,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'konveyer' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'konveyer' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_kon,
					   
					   (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'mesin' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS pln_mes,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'mesin' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS akt_mes,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'mesin' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'mesin' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_mes,
					   
					   (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'spray' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS pln_spr,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'spray' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS akt_spr,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'spray' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'spray' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_spr,
					   
					    (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'waterfall' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS pln_wat,
					 (select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'waterfall' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan) AS akt_wat,
					 truncate((select x.aktual from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'waterfall' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)/
					 (select x.plan from gl_trans x join fg_users y on x.pic = y.username where x.tgl_input = t.tgl_input
					   AND x.jenis = 'waterfall' and YEAR(x.tgl_input) = $tahun AND MONTH(x.tgl_input) = $bulan)*100,2) as ar_wat
					   
					FROM gl_trans t where YEAR(t.tgl_input) = $tahun AND MONTH(t.tgl_input) = $bulan
					group BY day(t.tgl_input)";
...
from dko_transaksi where kategori = 22 $tglklausa group by tgl_input";
	...
	*/
	...
	public function ambilBln($tgl){
        ...
        public function ambilBlnFull($tgl){
		$exp = explode('-',$tgl);
		$tgl = $exp[1];
		$bln = $this->model_global->getBulan($tgl);
		$hasil = substr($bln,0);
        ...
        public function ambilBlnOnly($tgl){
		$exp = explode('-',$tgl);
		$tgl = $exp[1];
		return $tgl;
	}
        
         public function ambilYerOnly($tgl){
		$exp = explode('-',$tgl);
		$tgl = $exp[2];
		return $tgl;
	}
	...
	public function getLoginData($usr,$psw)
	{
        ...
        function maxGlz(){
                    $q = $this->db->query("SELECT MAX(id_fg) as no FROM gl_trans");
                    foreach($q->result() as $dt){
                            $no = (int) $dt->no+1;
                    }
                    return $no;
        ...
        function getJenisFg(){
                $query="SELECT * from wip_keramik_kat where CURDATE() between period_start and period_end AND jns_wip like '%Pengiriman%'";
                $q=$this->db->query($query);    
                if ($q->num_rows() > 0){
                    foreach($q->result() as $row){
                        $data[]=$row;
        ...
        function getJenisFgUnlock(){
                $query="SELECT * from wip_keramik_kat where jns_wip like '%Pengiriman%'";
                $q=$this->db->query($query);    
                if ($q->num_rows() > 0){
                    foreach($q->result() as $row){
                        $data[]=$row;
